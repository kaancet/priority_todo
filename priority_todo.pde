...
color currentcolor;
RectButton addButton, clearButton, exitButton, taskButton, loadButton;
...
void ItemDialog()
{
  ...
  text("Due Date:",box_l+98,box_t+150);
  //input box
...
void controlEvent(ControlEvent theEvent) {
  if(theEvent.isAssignableFrom(Textfield.class)) {
    taskLabel=textCP5.get(Textfield.class,"").getText();
    countdown_string=dueCP5.get(Textfield.class,".").getText();
    countdown=int(Float.parseFloat(countdown_string));
    println(taskLabel);
    println(countdown);
...
void setup()
{
  ...
  loadButton = new RectButton("Load",width-120,30,100,44,buttoncolor,highlight,fontfill);
  ...
  if(mousePressed) {
    ...
    if(add_dialog == true){
      if(exitButton.pressed()){
        ...
        print("DONE!");
        add_dialog=false;
        remain_hour=getRemain(countdown);
        todos.add(new todoItem(taskLabel,remain_hour,itemno,0));
        itemno++;
        textCP5.remove("");
        dueCP5.remove(".");
      }
    ...
    if(addButton.pressed())
    {
      ...
      add_dialog=true;
       ...
       textCP5 = new ControlP5(this);
       textCP5.addTextfield("")
         .setPosition(width/2-x/2-150,height/2-60)
         .setSize(300,40)
         .setFont(font)
         .setFocus(true)
         .setColor(color(255));
         
       dueCP5 = new ControlP5(this);
       dueCP5.addTextfield(".")
         .setPosition(width/2-x/2-150,height/2+38)
         .setSize(300,40)
         .setFont(font)
         .setFocus(true)
         .setColor(color(255));
    ...
    else if(clearButton.pressed())
    {
      ...
      {
        todoItem c = (todoItem)todos.get(i);
        if(c.inZone == 10)
        {
          todos.remove(i);
          itemno--;
    ...
    {
      selectInput("Select Load File","loadItems");
      print("LOAD!");
    }
  }
